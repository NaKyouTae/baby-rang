...
const RELIABILITY_MSG =
  '이번 결과는 아기의 현재 모습보다는 응답 패턴의 영향이 클 수 있어요. 며칠 뒤 다시 진행해 보시면 더 안정적인 결과를 볼 수 있습니다.';
...
const FREE_CONTENT: Record<PrimaryType, FreeTypeContent> = {
  explorer: {
    title: '우리 아기는 몸으로 배우는 탐험가형 기질이 강해요.',
    description:
...
const PAID_TYPE_DETAIL: Record<PrimaryType, string> = {
  ...
  observer:
    '작은 차이도 놓치지 않고 세심하게 받아들이는 아기입니다. 겉으로는 조용해 보여도 내면의 감각과 관찰은 매우 풍부할 수 있습니다. 안전하고 편안한 환경에서 이 강점이 더 잘 드러납니다.',
  concentrator:
...
> = {
  activity: {
    high: {
      description:
        '이 아기는 움직임 욕구가 비교적 큰 편입니다. 가만히 오래 앉아 있는 상황보다 몸을 쓰는 활동에서 더 편안함을 느낄 수 있어요. 활동성은 산만함과 같은 뜻이 아니라, 에너지를 발산하는 방식의 차이로 이해하는 것이 좋습니다.',
      parentTips: [
  ...
  emotional_intensity: {
    high: {
      description:
        '기쁨도 슬픔도 분명하게 표현하는 편입니다. 감정이 또렷하기 때문에 아기의 상태를 읽기 쉬운 장점이 있지만, 감정 조절에는 더 많은 연습이 필요할 수 있어요.',
      parentTips: [
  ...
  sensitivity: {
    ...
    medium: {
      ...
      parentTips: ['아기가 불편해하는 감각이 있다면 세심하게 관찰해 주세요'],
    },
    low: {
      description:
...
const PAID_STRENGTHS: Record<PrimaryType, StrengthItem[]> = {
  explorer: [
    ...
    },
    {
      title: '표현의 생동감',
      description:
        '감정과 관심이 또렷하게 드러나기 때문에 아기의 상태를 읽기 쉬운 편입니다.',
    },
  ],
  socializer: [
    {
...
const PAID_CAUTIONS: Record<PrimaryType, StrengthItem[]> = {
  explorer: [
    ...
    },
    {
      ...
      description: '충분히 움직인 뒤에는 오히려 안정되는 아기도 많습니다.',
    },
    {
  ...
  balanced: [
    {
      ...
      description:
        '다양한 활동을 시도하며 아기가 더 반짝이는 영역을 찾아 주세요.',
    },
    {
    ...
    },
    {
      ...
      description: '아기에게 선택권을 자주 주면 선호가 더 잘 드러납니다.',
    },
  ],
};
...
const PARENT_ADVICE: Record<PrimaryType, string[]> = {
  explorer: [
    "이 아기는 '통제'보다 '리듬'이 중요할 수 있어요.",
    '먼저 진정시키고, 그다음 가르쳐 주세요.',
    '장점을 문제처럼 보지 않도록 해석의 방향을 바꿔보세요.',
  ],
  socializer: [
    '관계 속에서 배우는 아기예요. 함께하는 시간을 충분히 주세요.',
    '거절당했을 때 감정을 먼저 받아주세요.',
    '혼자 있는 시간도 편안하게 느낄 수 있도록 도와주세요.',
  ],
  observer: [
    '이 아기에게는 안전감이 가장 큰 자양분이에요.',
    '표현이 적다고 괜찮은 게 아닐 수 있어요.',
    '아기의 속도를 존중하면 용기가 자랍니다.',
  ],
  concentrator: [
    '몰입의 흐름을 존중해 주세요. 그게 이 아기의 강점이에요.',
...
const CLOSING_MESSAGE =
  '기질에는 좋은 것도 나쁜 것도 없어요. 중요한 것은 아기를 바꾸는 것이 아니라, 아기에게 맞는 방식으로 이해하고 도와주는 것입니다.';
