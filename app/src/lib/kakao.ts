...
// throughTalk: true → 모바일에서 카카오톡 앱으로 점프해 SSO 동의만 받음(계정 입력 없음).
// 앱 미설치 / 데스크톱이면 kauth.kakao.com 웹 로그인 페이지로 자동 폴백된다.
...
export async function authorizeWithKakao(opts: {
  ...
  sdk.Auth.authorize({ redirectUri: opts.redirectUri, throughTalk: true });
}
